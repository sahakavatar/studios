  ...
  editorTextContianer.getSession().setMode("ace/mode/css");
	...
	var classb = {
			sl:{
			 ...
			 setupstduio:{
				  "all": {
					"collection":["basicclasses","textcollection", "containercollections", "elementcollection"]
				  },
			 },
      activejson: {},
      selectorsss:{
        ...
        if(classb.generalSetting.csssetting['csskey']){
            var cssdats =  classb.generalSetting.csssetting['csskey'];
              var classel = classb.generalSetting.csssetting.classname
				...
				$('[data-export="collectiondata"]').val(classb.generalSetting.csssetting.savedcss)
...
$('body').on('change', '[data-switchStudio]', function(){
        ...
        $('[data-targetStudioType]').not('[data-targetStudioType="'+thisVal+'"]').collapse("hide")
        $('[data-targetStudioType="'+thisVal+'"]').collapse("show")
		if(thisVal=='custom'){
			$('[data-previewcolumn="option"]').collapse("hide")
		}else{
			$('[data-previewcolumn="option"]').collapse("show")
		}
